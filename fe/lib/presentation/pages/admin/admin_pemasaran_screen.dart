...
class _AdminPemasaranScreenState extends State<AdminPemasaranScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final pendingCount = orders
        .where((order) => order.status == OrderModel.statusPending)
        .length;
    final failedCount = orders
        .where((order) => order.status == OrderModel.statusCancelled)
        .length;
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 24),
                  SizedBox(height: 220, child: _MonthlyOrderBarChart()),
                ],
              ),
            ),
            const SizedBox(height: 24),
                      ...
                      rows: orders
                          ...
                          .map(
                            (order) => _orderRow(
                              ...
                              _formatRupiah(order.totalPrice.round()),
  ...
  DataRow _orderRow(
    ...
    return DataRow(
      cells: [
        DataCell(
          Text(id, style: const TextStyle(fontSize: 13, color: _textMuted)),
        ),
        DataCell(
          Text(
            buyer,
