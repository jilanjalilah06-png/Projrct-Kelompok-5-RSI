...
class OrderRepository {
  ...
  Future<OrderModel> createOrder({
    ...
    if (response.statusCode == 201 || response.statusCode == 200) {
      ...
      data['snap_token'] ??= body['snap_token'];
  ...
  Future<OrderModel> updateOrderStatus(
    ...
    if (buyerReview != null) {
      fields['buyer_review'] = buyerReview;
    }
    if (buyerRating != null) {
      fields['buyer_rating'] = buyerRating.toString();
    }
    ...
    if (deliveryProof != null) {
      ...
      final response = await _apiService.postMultipartRequest(
        ...
        files: {
          'delivery_proof': multipartFile,
        },
      );

      if (response.statusCode == 200) {
        final respStr = await response.stream.bytesToString();
        final Map<String, dynamic> body = jsonDecode(respStr);
        final Map<String, dynamic> data = body['data'];
        return OrderModel.fromJson(data);
      } else {
        throw Exception(
  ...
  Future<OrderModel> createPayment(int orderId) async {
    final response = await _apiService.postRequest(
      '${ApiConstants.ordersEndpoint}/$orderId/payment',
      {},
    );
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      final Map<String, dynamic> body = jsonDecode(response.body);
      throw Exception(
        body['message'] ?? 'Gagal membuat pembayaran Midtrans: ${response.statusCode}',
      );
    }
  }

  Future<OrderModel> simulatePayment(int orderId) async {
    final response = await _apiService.postRequest(
      '${ApiConstants.ordersEndpoint}/$orderId/simulate-payment',
      {},
    );
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      final Map<String, dynamic> body = jsonDecode(response.body);
      throw Exception(
        body['message'] ?? 'Gagal menjalankan simulasi pembayaran: ${response.statusCode}',
      );
    }
  }
